...
```bash
# Run full evaluation suite (26 cases: 15 visible + 11 original regression cases)
python main.py --eval
# or directly:
...
# Optional: LLM Provider Selection (openai / gemini)
LLM_PROVIDER=openai
...
| --- | --- | --- |
| **Model Generation Layer** | OpenAI API (with Explicit `LLM_PROVIDER` & Offline Fallback) | LLM composition layer uses OpenAI API when configured, and falls back to deterministic grounded evidence composition when running offline or in unit tests. |
| **Embedding / Retrieval** | Dual FeatureUnion TF-IDF (Word (1,3) + Sub-word Char (3,5) N-Grams) + Domain Paraphrase Expansion | Deterministic, zero external database latency, lightweight, 100% reproducible locally without vector DB overhead. Maps natural user paraphrases (e.g., `send back` → `return policy`, `Toronto` → `Canadian shipping`) to canonical domain concepts. |
...
- `evaluation/evaluate.py`: Automated evaluation harness testing accuracy across 26 scenarios.
...
| `privacy` | 3 | 3 | **100.0%** | Refusing to expose PII (email, address) and internal fields even when order ID is present. |
...
- **Fix:** Updated `IntentRouter.route()` to check prompt injection and explicit privacy patterns BEFORE order ID extraction. Explicit PII requests are now routed to `PRIVACY_REQUEST` (`tool_called = None`) and refused.
...
5. Evaluation Suite Execution:
   $ python evaluation/evaluate.py
   EVALUATION SUMMARY: 26/26 Passed (100.0%)
```
...
- `evaluation/original-cases.json`: 11 original evaluation regression cases.
